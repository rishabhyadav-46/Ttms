...
app.get('/export-csv', async (req,res)=>{
  const entry = await GateEntry.find({}).select("gateEntryNo vehicleNumber transporter vehicleType  inwardType location reportingTimeDate inDateTime tareWeight status").lean();

   const cleanData = entry.map(item =>({
    ...
    ReportingDateTime : item.reportingTimeDate,
    ...
    const csv = json2csv.parse(cleanData);

  res.header("Content-Type","text/csv");
  res.attachment("EntryDetail.csv");
  return res.send(csv);
})
